...
type BundleConfig() =
    static member RegisterBundles (bundles:BundleCollection) =
        ...
        bundles.Add(ScriptBundle("~/bundles/vendor").Include(
                                     "~/Scripts/bootstrap.*", 
                                     "~/Scripts/knockout-3.*"))
...
type ApiRouteCat = { id : RouteParameter; cat: RouteParameter; dept: RouteParameter}
...
type Global() =
    ...
    static member RegisterRoutes(routes:RouteCollection) =
        ...
        routes.MapHttpRoute( "DefaultApi", "api/{controller}/{id}/{cat}/{dept}", 
            { id = RouteParameter.Optional; cat = RouteParameter.Optional; dept = RouteParameter.Optional } ) |> ignore
